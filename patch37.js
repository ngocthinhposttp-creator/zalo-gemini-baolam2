var oldExtractFile=extractFile, oldMeta=verifiedPdfMeta, oldClassify=classifyItem, oldScore=scoreFolder;
  var V='3.7 TESTED Header Vision';
  ...
  async function tess(c,label,opt){
    ...
    var r=await Tesseract.recognize(c,'eng',{
      tessedit_pageseg_mode:String(opt.psm||6),
      tessedit_char_whitelist:opt.whitelist||''
    });
  ...
  function componentCount(mask){
    var ctx=mask.getContext('2d',{willReadFrequently:true}),im=ctx.getImageData(0,0,mask.width,mask.height),d=im.data,w=mask.width,h=mask.height,seen=new Uint8Array(w*h),count=0;
    var qx=new Int32Array(w*h),qy=new Int32Array(w*h);
    for(var y=Math.floor(h*.42);y<h;y++)for(var x=0;x<w;x++){
      var idx=y*w+x;if(seen[idx]||d[idx*4]>80)continue;
      var qs=0,qe=0,area=0,minx=x,maxx=x,miny=y,maxy=y;qx[qe]=x;qy[qe++]=y;seen[idx]=1;
      while(qs<qe){
        var cx=qx[qs],cy=qy[qs++];area++;if(cx<minx)minx=cx;if(cx>maxx)maxx=cx;if(cy<miny)miny=cy;if(cy>maxy)maxy=cy;
        for(var yy=Math.max(0,cy-1);yy<=Math.min(h-1,cy+1);yy++)for(var xx=Math.max(0,cx-1);xx<=Math.min(w-1,cx+1);xx++){
          var ni=yy*w+xx;if(!seen[ni]&&d[ni*4]<=80){seen[ni]=1;qx[qe]=xx;qy[qe++]=yy}
        }
      }
      var ww=maxx-minx+1,hh=maxy-miny+1;if(area>120&&hh>24&&ww>10)count++;
    }
    return count;
  }
  function consensusNumber(rowText,blueText,n){
    var suf=suffixFrom(rowText),blue=(String(blueText||'').match(/\d/g)||[]).join('');
    if(suf&&n>=1&&n<=7&&blue.length===n)return blue+'/'+suf;
    var direct=numFromText(rowText);if(direct){
      var d=(direct.match(/^\d+/)||[])[0]||'';
      if(!n||d.length===n)return direct;
    }
    return '';
  }

  async function headerVision(pdf){
    var page=await renderFirst(pdf),
        top=crop(page,0,0,1,.33),
        leftRow=crop(page,0,.045,.46,.115),
        blue=blueMask(leftRow);
    var full=await tess(top,'BL2 header 300dpi',{psm:4}),
        row=await tess(leftRow,'BL2 số ký hiệu',{psm:6}),
        digits=await tess(blue,'BL2 mực viết tay',{psm:6,whitelist:'0123456789'});
    var ht=plain(full&&full.text||''),rt=one(row&&row.text||''),bt=one(digits&&digits.text||''),
        n=componentCount(blue),no=consensusNumber(rt,bt,n);
    if(!no)no=numFromText(rt)||numFromText(ht);
  ...
  extractFile=async function(file){
  ...
  verifiedPdfMeta=function(text,name){
  ...
  if(typeof oldScore==='function')scoreFolder=function(folder,profile,name){
  ...
  classifyItem=async function(item){
